...
use crate::pre::{ProxyReEncryption, PreKeyPair, EncryptedPreData};
...
mod tests {
    ...
    fn test_transfer_proof_serialization() {
        ...
        // Шифруем для sender
        let encrypted = pre.encrypt(&sender.public_key(), aes_key.as_bytes()).unwrap();
        
        // Создаём transfer proof
        let service = TransferService::new(&pre);
        let proof = service.create_transfer_proof(
            &encrypted,
            &sender,
    ...
    fn test_full_transfer_flow() {
        ...
        let encrypted_for_alice = pre.encrypt(&alice.public_key(), aes_key.as_bytes()).unwrap();
        ...
        let (received_key, is_valid) = service.accept_transfer(
            &proof,
            &bob,
    ...
    fn test_invalid_commitment_detected() {
        ...
        assert!(!commitment.verify(&wrong_key), "Wrong key should not verify");
    }
}
