...
use xrpl_vault_crypto_core::{
    ...
    AesKey, EncryptedPreData,
    KeyCommitment, TransferProof, TransferService,
};
...
mod tests {
    ...
    fn test_encrypt_decrypt_aes_key() {
        ...
        let encrypted = transfer.encrypt_aes_key(&aes_key, &keypair.public_key()).unwrap();
    ...
    fn test_full_transfer_flow() {
        ...
        let aes_key = AesKey::generate();
        let commitment = transfer.create_commitment(&aes_key);
        let encrypted_for_alice = transfer.encrypt_aes_key(&aes_key, &alice.public_key()).unwrap();
        
        // Alice передаёт Bob
        let proof = transfer.re_encrypt_for_recipient(
            &encrypted_for_alice,
            &alice,
        ...
        // Bob принимает
        let (received_key, is_valid) = transfer.accept_transfer(
            &proof,
            &bob,
