...
interface FilesScreenProps { onNavigate:(s:'files'|'upload'|'settings')=>void; searchQuery?:string; oracleConnected?:boolean; localWalletAvailable?:boolean }
...
export default function FilesScreen({onNavigate,searchQuery='',localWalletAvailable=true}:FilesScreenProps) {
      ...
      <div style={{maxWidth:1200,margin:'0 auto'}}>
        ...
        {filtered.length===0?(
            <div style={{textAlign:'center',padding:'80px 20px',background:'var(--bg-2)',borderRadius:'var(--radius-lg)',overflow:'visible',border:'1px solid var(--line)'}}>
              ...
              <h3 style={{fontSize:18,fontWeight:600,color:'var(--fg-0)',margin:'0 0 8px'}}>{localWalletAvailable?'No files yet':'Restore local Vaulted wallet'}</h3>
              <p style={{fontSize:13,color:'var(--fg-2)',margin:'0 0 20px'}}>
                {localWalletAvailable
                    ? 'Upload your first file to get started'
                    : 'This Oracle session is approved, but local files and NFT ownership are tied to the 12-word Vaulted wallet on this device. Restore the matching wallet to continue.'}
              </p>
              {localWalletAvailable && (
